...
import com.forge.task.ExecutionResults
import com.forge.task.Task
import com.forge.task.TaskExecutionPlan
import com.forge.task.TaskResult
import com.forge.task.TaskStatus
...
class RemoteExecutionExecutor(
    ...
    fun execute(executionPlan: TaskExecutionPlan, verbose: Boolean = false, skipCache: Boolean = false): ExecutionResults = runBlocking {
        ...
        for ((layerIndex, layer) in executionPlan.layers.withIndex()) {
            ...
            layerResults.forEach { result ->
                results[result.task.id.value] = result
            }
            ...
            if (failedTasks.isNotEmpty()) {
                ...
                failedTasks.forEach { result ->
                    logger.error("Failed task: ${result.task.id.value} - ${result.error}")
                }
    ...
    private suspend fun executeTask(task: Task, verbose: Boolean, skipCache: Boolean): TaskResult {
        ...
        try {
            logger.info("Executing remote task: ${task.id.value}")
            
            val projectNode = projectGraph.nodes[task.project]
            if (projectNode == null) {
                logger.error("Project not found: ${task.project}")
                return TaskResult(
                    task = task,
                    status = TaskStatus.FAILED,
                    startTime = startInstant,
                    endTime = Instant.now(),
                    error = "Project not found: ${task.project}"
                )
            }
            ...
            if (!skipCache && task.isCacheable()) {
                val cachedResult = checkActionCache(executeRequest.actionDigest)
                if (cachedResult != null) {
                    logger.info("Task ${task.id.value} found in cache")
                    return TaskResult(
                        task = task,
            ...
            logger.info("Uploading action blobs to CAS for task: ${task.id.value}")
            ...
            if (!operation.done) {
                logger.error("Remote execution did not complete: ${task.id.value}")
                return TaskResult(
                    task = task,
                    status = TaskStatus.FAILED,
                    startTime = startInstant,
            ...
            if (operation.hasError()) {
                logger.error("Remote execution failed for task ${task.id.value}: ${operation.error}")
                return TaskResult(
                    task = task,
                    status = TaskStatus.FAILED,
                    startTime = startInstant,
            ...
            if (result.exitCode == 0) {
                logger.info("Remote task ${task.id.value} completed successfully in ${duration}ms")
                ...
                logger.error("Remote task ${task.id.value} failed with exit code ${result.exitCode}")
                return TaskResult(
                    task = task,
                    status = TaskStatus.FAILED,
                    startTime = startInstant,
            ...
            logger.error("Remote execution failed for task ${task.id.value}: ${e.message}", e)
            return TaskResult(
                task = task,
                status = TaskStatus.FAILED,
                startTime = startInstant,
            ...
            logger.error("Unexpected error executing task ${task.id.value}: ${e.message}", e)
            return TaskResult(
                task = task,
                status = TaskStatus.FAILED,
                startTime = startInstant,
    ...
    private suspend fun uploadActionBlobs(task: Task, projectRoot: String, actionDigest: Digest) {
        try {
            ...
            logger.info("Uploaded ${response.responsesList.size} blobs to CAS for task: ${task.id.value}")
            
        } catch (e: Exception) {
            logger.error("Failed to upload blobs for task ${task.id.value}: ${e.message}", e)
            throw e
        }
    }
