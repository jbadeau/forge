    ...
    private fun findDaemonJar(): Path {
        ...
        for (clientDir in possibleClientDirs) {
            possibleLocations.addAll(listOf(
                clientDir.resolve("frontseat.jar"),
                clientDir.resolve("frontseat/target/frontseat.jar"),
                clientDir.resolve("../frontseat/target/frontseat.jar"),
                clientDir.resolve("frontseat-github/frontseat/target/frontseat.jar")
            ))
        }
        
        // Add system location
        possibleLocations.add(Paths.get(System.getProperty("user.home"), ".forge", "daemon", "frontseat.jar"))
        ...
        throw RuntimeException("Could not find frontseat.jar. Tried: ${possibleLocations.joinToString(", ")}")
    }
