...
import com.forge.project.ProjectGraphNode
import com.forge.project.DiscoveryPlugin
import com.forge.task.TaskGraph
    ...
    companion object {
        ...
        fun discover(startPath: Path = Paths.get("").toAbsolutePath()): Workspace {
            val workspaceRoot = findWorkspaceRoot(startPath)
                ?: throw IllegalStateException("Could not find workspace root (no forge.json found)")
        ...
        fun create(workspaceRoot: Path): Workspace {
            ...
            val configPath = workspaceRoot.resolve(".forge").resolve("workspace.yml")
                .takeIf { it.exists() } 
                ?: workspaceRoot.resolve("forge.json")
                    .takeIf { it.exists() }
        ...
        /**
         * Find workspace root by looking for forge.json or .forge directory
         */
        private fun findWorkspaceRoot(startPath: Path): Path? {
            var current = startPath
            
            while (true) {
                // Look for forge.json or .forge/workspace.yml
