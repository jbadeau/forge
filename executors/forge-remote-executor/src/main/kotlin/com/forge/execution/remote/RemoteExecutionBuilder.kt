...
class RemoteExecutionBuilder(
    ...
    fun buildAction(task: Task, projectRoot: String): Action {
        ...
        return Action.newBuilder()
            ...
            .setDoNotCache(!task.target.isCacheable())
            .build()
    }
    ...
    fun buildCommand(task: Task, projectRoot: String): Command {
        ...
        val commands = extractCommands(task.target)
        ...
        val workingDir = resolveWorkingDirectory(task.target, projectRoot)
        commandBuilder.setWorkingDirectory(workingDir)
        
        // Add environment variables
        val envOptions = task.target.options["env"] as? Map<*, *> ?: emptyMap<String, String>()
        ...
        task.target.outputs.forEach { output ->
            val resolvedOutput = resolvePathPattern(output, task.projectName, projectRoot)
    ...
    fun buildInputRoot(task: Task, projectRoot: String): Directory {
        ...
        task.target.inputs.forEach { input ->
            val resolvedPaths = resolveInputPattern(input, task.projectName, projectRoot)
